
JLOP Chapter 7. Forms

From the website:

  Java LibreOffice Programming
  http://fivedots.coe.psu.ac.th/~ad/jlop

  Dr. Andrew Davison
  Dept. of Computer Engineering
  Prince of Songkla University
  Hat yai, Songkhla 90112, Thailand
  E-mail: ad@fivedots.coe.psu.ac.th


If you use this code, please mention my name, and include a link
to the website.

Thanks,
  Andrew

============================

This directory contains 3 Java files:
  * BuildForm.java, ExamineForm.java, FormOutliner.java

/* I don't describe FormOutliner.java in the chapter since it uses a 
   variant of the code in ExamineForm.java, but moved into Forms.listForms()
*/


5 test files for the Java code:
  * form1.odt, scratchExample.odt, build.odt
  * liang.odb
  * smiley.png  (currently unused)


A list of XEventListener subclasses:
  * XEventListenerSubclasses.txt
        // useful when you want to do a search for a listener

There are 7 batch files:
  * compile.bat
  * run.bat
     - make sure they refer to the correct locations for your
       Java, JNA, and my Utils classes.
       For details, read:
         "Installing the code for "Java LibreOffice Programming"
          http://fivedots.coe.psu.ac.th/~ad/jlop/install.html

    compile.bat and run.bat file both call:

  * lofind.bat
     - this tries to find LibreOffice (v4 or v5) on your machine,
       and checks its "bitness" with that of Java. They should
       both either be 32-bit or 64-bit. A warning is printed if 
       they are not.

  * loKill.bat        // kills the Office process
  * loList.bat        // lists the Office process (if it is running)

  * loDoc.bat         // searches the online LibreOffice documentation
  * loGuide.bat       // searches the online OpenOffice Dev Guide



----------------------------
Compilation

> compile *.java
    // you must have LibreOffice, Java, JNA, and my Utils classes installed


===================================
Execution:

// You must have LibreOffice, Java, JNA, and my Utils classes installed


> run BuildForm
     // when the user presses enter at the command prompt, the form 
     // (minus the listeners) is saved as "build.odt"


> run ExamineForm <an ODT file>
e.g. run ExamineForm build.odt
     // when the user presses enter at the command prompt, the program closes 


> run FormOutliner <an ODT file>
e.g. run FormOutliner scratchExample.odt


=================================
Last updated: 14th June 2016
